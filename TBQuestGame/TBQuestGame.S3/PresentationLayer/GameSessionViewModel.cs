...
namespace WpfTheAionProject.PresentationLayer
{
    ...
    public class GameSessionViewModel : ObservableObject
    {
        ...
        private void ProcessItemUse(Items item)
        {
            string message;
            ...
            switch (item.UseAction)
            {
                case Items.UseActionType.OPENLOCATION:
                    message = _gameMap.OpenLocationsByRelic(item.Id);
                    CurrentLocationInformation = item.UseMessage;
                    break;
                case Items.UseActionType.KILLPLAYER:
                    OnPlayerDies(item.UseMessage);
                    break;
                default:
                    break;
            }
